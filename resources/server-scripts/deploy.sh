...
if [ ${INCLUDE_DATABASE} = true ]; then
  ...
  if docker ps -a | grep -q ${VOLUME_CONTAINER_NAME}; then
    ...
    if [ ${DRY_RUN} = false ]; then
      docker run -d --volumes-from ${VOLUME_CONTAINER_NAME} --net=${ELOPHANT_NETWORK} -p 5432:5432 -e POSTGRES_PASSWORD=${ELOPHANT_ADMIN_PASSWORD} -e CONSUL_CLIENT_ADDRESS=elophant_consul_client:8500 -e CONSUL_SERVICE_ADDRESS=${DATABASE_NAME} -e CONSUL_SERVICE_PORT=5432 -e CONSUL_SERVICE_NAME=elophant-db --name ${DATABASE_NAME} ${REPOSITORY}/${DB_IMAGE}:latest
    else
      echo "Dry Run: 'docker run -d --volumes-from ${VOLUME_CONTAINER_NAME} --net=${ELOPHANT_NETWORK} -p 5432:5432 -e RUN_INIT_SCRIPTS=false -e POSTGRES_PASSWORD=${ELOPHANT_ADMIN_PASSWORD} -e CONSUL_CLIENT_ADDRESS=elophant_consul_client:8500 -e CONSUL_SERVICE_ADDRESS=${DATABASE_NAME} -e CONSUL_SERVICE_PORT=5432 -e CONSUL_SERVICE_NAME=elophant-db --name ${DATABASE_NAME} ${REPOSITORY}/${DB_IMAGE}:latest'"
    fi
  else
    ...
    if [ ${DRY_RUN} = false ]; then
      docker run -d --volumes-from ${VOLUME_CONTAINER_NAME} --net=${ELOPHANT_NETWORK} -p 5432:5432 -e POSTGRES_PASSWORD=${ELOPHANT_ADMIN_PASSWORD} -e ELOPHANT_USER_PASSWORD=${ELOPHANT_USER_PASSWORD} -e CONSUL_CLIENT_ADDRESS=elophant_consul_client:8500 -e CONSUL_SERVICE_ADDRESS=${DATABASE_NAME} -e CONSUL_SERVICE_PORT=5432 -e CONSUL_SERVICE_NAME=elophant-db --name ${DATABASE_NAME} ${REPOSITORY}/${DB_IMAGE}:latest
      sleep 10
    else
      echo "Dry Run: 'docker run -d --volumes-from ${VOLUME_CONTAINER_NAME} --net=${ELOPHANT_NETWORK} -p 5432:5432 -e RUN_INIT_SCRIPTS=true -e POSTGRES_PASSWORD=${ELOPHANT_ADMIN_PASSWORD} -e ELOPHANT_USER_PASSWORD=${ELOPHANT_USER_PASSWORD} -e CONSUL_CLIENT_ADDRESS=elophant_consul_client:8500 -e CONSUL_SERVICE_ADDRESS=${DATABASE_NAME} -e CONSUL_SERVICE_PORT=5432 -e CONSUL_SERVICE_NAME=elophant-db --name ${DATABASE_NAME} ${REPOSITORY}/${DB_IMAGE}:latest'"
    fi
  fi
fi
